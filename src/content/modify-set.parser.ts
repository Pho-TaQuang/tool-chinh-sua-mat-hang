import type { ClipboardPreview, ParsedClipboardRow } from "./modify-set.types";
  ...
  for (let lineIndex = 0; lineIndex < lines.length; lineIndex += 1) {
    ...
    if (nonEmptyCells.length === 0) {
      rows.push({
        ...
        priceInput: "",
        costInput: "",
        errors: ["Dòng trống hoặc không có dữ liệu hợp lệ."]
      });
      continue;
    }

    const nameCell =
      nonEmptyCells.find((entry) => !isNumericToken(entry.value)) ??
      null;
    const name = nameCell?.value ?? "";
    if (!name) {
      errors.push("Thiếu tên option.");
    }
    ...
    for (const cell of nonEmptyCells) {
      ...
      if (isNumericToken(cell.value)) {
        ...
        errors.push(`Giá trị không phải số: \"${cell.value}\".`);
      }
    }
